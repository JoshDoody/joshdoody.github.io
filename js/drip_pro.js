...
*/
      ...
      */
        ...
        jQuery(window).scroll(function() {
          ...
          */
          ...
          if ((docheight - wintop) < 900) {
            ...
            if (!has_scrolled) {
              if (this.settings.mixpanel) mixpanel.track('Ad - Slider')
              has_scrolled = true
            }
      ...
      modal: function(intent, content, options) {
        ...
        function handleMouseLeave(e) {
          ...
          _delayTimer = setTimeout(function(){
            ...
            if (this.settings.mixpanel) mixpanel.track('Ad - Modal', { instant: false })
            ...
            modal.on('modal:after-close', function(){
              ...
              if (this.settings.mixpanel) mixpanel.track('Ad - Modal Close', { instant: false })
      ...
      instamodal: function(intent, content) {
        ...
        window._delayTimer = setTimeout(function(){
          if (this.settings.mixpanel) mixpanel.track('Ad - Modal', { instant: true })
          var modal = jQuery('<div class="modal">')
                        .data('placeholder', intent)
                        .html(content)
                        .appendTo('body')
                        .modal()
          modal.on('modal:after-close', function(){
            if (this.settings.mixpanel) mixpanel.track('Ad - Modal Close', { instant: true })
            t.storage.set(cookie_name, '1')
          })
        }, m_delay)
    ...
    */
    ...
    DripPro.prototype.modify_forms = function() {
      ...
      jQuery('[data-drip-embedded-form]').each(function(){
        ...
        var affiliate_id = t.storage.get('affwp_ref')
    ...
    */
    DripPro.prototype.set_affiliate = function() {
      if (this.storage.get('appwp_ref')) {
        if (!this.anon && this.drip_contact.custom_fields && !this.drip_contact.custom_fields.affiliate_id) {
          _dcq.push(['identify', { affiliate_id: this.storage.get('appwp_ref')} ])
        }
        return
      }

      if (!this.is_anon && this.drip_contact.custom_fields && this.drip_contact.custom_fields.affiliate_id) {
        this.storage.set('appwp_ref', this.drip_contact.custom_fields.affiliate_id)
        this.storage.set('affwp_ref_visit_id', this.drip_contact.custom_fields.affiliate_id)
      }
    }
    ...
    */
    DripPro.prototype.track = function() {
      if (window.mp && this.settings.mixpanel) {
        ...
        if (this.is_anon) {
          ...
          mixpanel.people.set(people_options)
        }

        mixpanel.register(options)
        mixpanel.track(window.mp[0], window.mp[1])

        jQuery('[rel="modal:open"]').click(function(e){
          var popup = jQuery(this).attr('href')
          mixpanel.track('Optin - Modal', jQuery.extend(window.mp[1], { popup: popup }))
        })
        ...
        jQuery(document).on('submit.gf', '.gform_wrapper form', function(e){
          ...
          if (email) {
            ...
            mixpanel.track('Gravity Form Submitted', fields)
        ...
        jQuery(document).on('submit.drip', '[data-drip-embedded-form]', function(e){
          ...
          if (email) {
            ...
            mixpanel.track('Lead Form Submitted', fields)
    ...
    */
    DripPro.prototype.score_content = function(drip_contact) {
      ...
      var scoring = window[this.settings.scoring_obj]

      var identify = { tags: ["Read - " + scoring.name] }
      var category_downcase = scoring.category.toLowerCase().replace(/ /g, '_')
      var cookie = this.settings.cookie_prefix + '_cat_' + category_downcase

      var posts = ((drip_contact.custom_fields ? drip_contact.custom_fields[category_downcase] : false) || this.storage.get(cookie) || '').split(',')
      posts.push(scoring.id)
      ...
      var score = (f_posts.length / scoring.posts_in_cat) * 100

      this.storage.set(cookie, serialized)
      this.storage.set(cookie + '_score', score)
    ...
    */
    DripPro.prototype.push_survey = function() {
      ...
      jQuery('#survey-container .survey-why span').tooltip()
      ...
      jQuery('#survey-container li').click(function(e){
        ...
        mixpanel.people.set(identify)
        t.storage.set('survey_timer', 1, 259200) // 3 days
        if (this.settings.mixpanel) mixpanel.track('Survey - Completed', { 'survey question': plinko.attr, 'survey answer': value })
      ...
      jQuery('#survey-container .close-survey').click(function(e){
        ...
        if (this.settings.mixpanel) mixpanel.track('Survey - Closed', { 'survey question': plinko.attr })
      })
      ...
      jQuery(window).scroll(function() {
        ...
        if ((docheight - wintop) < 900) {
          ...
          if (!has_scrolled) {
            has_scrolled = true
            if (this.settings.mixpanel) mixpanel.track('Survey - Shown', { 'survey question': plinko.attr })
          }
          dom.addClass('expanded')
        }
      }.bind(this))
    ...
    DripPro.prototype.drip_response_received = function(payload) {
      ...
      if (!this.is_anon) {
        this.storage.set(this.settings.cookie_prefix + '_drip_id', payload.email)
      }
      ...
      this.track()
      this.replace()
      if (window[this.settings.scoring_obj]) this.score_content(payload)
